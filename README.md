...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/pypi_stats_recent \
  -H 'Content-Type: application/json' \
  -d '{"package":"requests"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/pypi_stats_recent`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "pypi-stats": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-pypi-stats"]
    }
  }
}
```
